    ...
    private Point2D nearest(Point2D p, Point2D currentNearestPoint, Node n) {
        ...
        if (n.nodeLevel % 2 == 0) {
            if (p.x() > n.data.x()) { // check right sub-tree
                Point2D npr = nearest(p, n.data.distanceTo(p) < currentNearestPoint.distanceTo(p) ?
                                         n.data : currentNearestPoint, n.right);
                ...
                Point2D npl = nearest(p, n.data.distanceTo(p) < currentNearestPoint.distanceTo(p) ?
                                         n.data : currentNearestPoint, n.left);
            ...
            if (p.y() > n.data.y()) { // check up sub-tree
                Point2D npu = nearest(p, n.data.distanceTo(p) < currentNearestPoint.distanceTo(p) ?
                                         n.data : currentNearestPoint, n.right);
                ...
                Point2D npd = nearest(p, n.data.distanceTo(p) < currentNearestPoint.distanceTo(p) ?
                                         n.data : currentNearestPoint, n.left);
